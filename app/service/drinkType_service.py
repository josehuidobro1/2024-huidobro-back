...
def getUserDrinkTypes(id_user):
    try:
        user_drinkType_query = db.collection(
            'DrinkType').where('id_user', '==', id_user)
        user_drinkType = user_drinkType_query.stream()

        default_drinkType_query = db.collection(
            'DrinkType').where('id_user', '==', 'default')
...
def deleteDrinkType(drinktype_id):
    try:
        ...
        collection_ref = db.collection(['Drink']).where(
            'typeOfDrink', '==', drinktype_id).stream()
